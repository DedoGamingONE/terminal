...
[[nodiscard]] Microsoft::WRL::ComPtr<IDWriteTextFormat> DxFontRenderData::TextFormatWithAttribute(DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style)
{
    return _textFormats[style != DWRITE_FONT_STYLE_NORMAL][weight != _userFontWeight];
}

[[nodiscard]] Microsoft::WRL::ComPtr<IDWriteFontFace1> DxFontRenderData::FontFaceWithAttribute(DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style)
{
    return _fontFaces[style != DWRITE_FONT_STYLE_NORMAL][weight != _userFontWeight];
}

gsl::span<DWRITE_FONT_AXIS_VALUE> DxFontRenderData::GetAxisVector(DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style)
{
...
#pragma warning(suppress : 26494) // Variable 'standardAxes' is uninitialized. Always initialize an object (type.5).
        ...
        for (auto italic = 0; italic < 2; ++italic)
        {
            ...
            {
                const auto fontWeight = bold ? DWRITE_FONT_WEIGHT_BOLD : static_cast<DWRITE_FONT_WEIGHT>(desired.GetWeight());
                ...
                const auto font = _ResolveFontWithFallback(desired.GetFaceName(), fontWeight, fontStyle);

                Microsoft::WRL::ComPtr<IDWriteFontFace> face;
                THROW_IF_FAILED(font->CreateFontFace(&face));
                THROW_IF_FAILED(face.As(&_fontFaces[italic][bold]));

                THROW_IF_FAILED(_dwriteFactory->CreateTextFormat(desired.GetFaceName().c_str(), _nearbyFontCollection.Get(), font->GetWeight(), font->GetStyle(), DWRITE_FONT_STRETCH_NORMAL, _fontSize, _userLocaleName.get(), &textFormat));
